...
# Data
Data for text classification taska are out there but not in very interesting topics. Stanford Uni has collected some [Amazon reviews](http://snap.stanford.edu/data/amazon/productGraph/categoryFiles/)
```{r message=FALSE, warning=FALSE}
library(jsonlite)
library(dplyr)
...
predictions_en <- make_prediction_en(test$reviewText)

rownames(test)=NULL

test %>% add_rownames() %>% mutate(rowname=as.numeric(rowname)) %>% 
  left_join(., predictions_en, by=c("rowname"="doc_id"))


make_prediction_nl(test$reviewText)
